...
#### 对象失去引用，怎么逃出被回收的命运
- 当一个对象失去了引用至少要经历2次标记过程：当根搜索后发现对象没有引用链个GC Roots相连它将进行第一次标记进行一次筛选（筛选的条件是复写了finalize方法，或者虚拟机还没有执行了finalize方法，就筛选）。筛选如果有必要执行finalize方法，会将对象放到一个T-queue的队列，然后由虚拟机创建的低优先级的Finalizer的线程去触发队列中的对象的finalize方法（只触发，不会等待方法执行完）。如果在执行finalize方法时对象重新和引用链上对象建立了关系，就能被移除即将回收的集合，完成自救，不被回收。当执行finalize方法后下次如果再被回收就不能完成自救，会被回收。（这个方法用于了解，具体时最好不要用）
#### 3.标记-清除算法
- 定义：标记阶段：从根对象向下标记所有可达的对象，未被标记的对象就是垃圾对象。清除阶段：清除未被标记的对象。
- 缺点：清除后的对象所留下的空间，不会被和在一起，因此回收后的空间不连续，会产生空间碎片，当需要分配一个较大的对象，无法找到连续的空间，不得不再一次触发垃圾回收。还有就效率的问题，标记和清除的效率都很低。
#### 4.复制算法
- 定义：将内存分为两块相等的部分，每次只使用其中一块，当这一块内存使用完或者进行垃圾回收时进行就将活着的对象复制到另一块空未使用的内存，然后清除原来使用的内存的全部对象，两部分交换角色。
- 优点：效率较高，不会产生空间碎片。
- 缺点:将内存分为了两部分，代价较高了点。
- 应用
![应用](https://github.com/myismyself/book_tip_jvm/blob/master/copy.png)
新生代产生的垃圾对象多，每次复制的对象较少，常运用在新生代
#### 5.标记-压缩
- 定义：从根节点开始向下，对可达的对象进行标记。然后压缩阶段将所有标记对象压缩到内存的一端，之后清理边界外所有对象。即避免了产生内存碎片，又不会将内存分为两部分性价比较高。常用于老年代这种存活对象的内存空间。
#### 6.增量算法
- 对于大部分垃圾回收算法来说，垃圾回收的过程中都处于stop the world 的状态，所有应用线程都必须挂起等垃圾回收线程执行完。如果垃圾回收执行时间很久，则应用程序就会被挂起来很久，影响用户体验。这个时候增量算法就是让垃圾回收线程和应用程序线程交替执行，每次垃圾回收线程只收集一小片内存区域，接着切换到应用线程直到垃圾收集完成。停顿少了，对用户体验提升了，但是是的回收的总体成本提升了，系统吞吐量下降。
#### 7.分代执行算法
- 根据不同内存区间的对象特点 执行不同的垃圾回收算法。老年代（标记-清除，标记-压缩）  新生代（复制算法）。
### 二、垃圾回收器
#### 垃圾回收器的分类
- 按线程数分为串行垃圾回收器（一次只开启一个线程进行垃圾回收）和并行垃圾回收器（一次开启多个线程同时进行垃圾回收）。
- 按工作模式分为独占式的垃圾回收器（一旦运行就停止其他应用程序线程运行直到垃圾回收结束）和并发式垃圾回收器（垃圾回收和应用程序线程交替执行）
...
##### 4.serial old 收集器
- 老年代垃圾收集器 单线程，标记-压缩算法
##### 5.parallel old 收集器
- 老年代 多线程垃圾收集器，标记-压缩算法
##### 6.cms收集器
- 以获取最短回收停顿时间为目的，采用标记-整理的算法。垃圾回收和应用程序线程交替执行。
- 起运作包含4个过程：1、初始标记：仅仅标记gc roots 能够直接关联的对象，速度快 。 2、并发标记：进行GC roots Tracing的过程。3、重新标记：修正并发标记期间，因用户程序继续运行而导致标记产生变动的那一部分对象的标记记录。这一阶段时间比初始标记要长些，比并发标记要短。4、并发清除。
- cms的缺点 1.对cpu资源十分敏感，默认启动的垃圾回收线程数为（cpu数量+1）/4 ,因此cpu较少时 对系统的影响教大 2.无法处理浮动垃圾，![图示](https://github.com/myismyself/book_tip_jvm/blob/master/jvm_book_img/cms_float_laji.jpg)
3.采用标记-清除算法，会产生内存碎片。
##### 7. G1收集器
- 基于 标记-压缩算法，非常准确的控制垃圾回收的停顿。
- 基本上在不牺牲吞吐量的情况下完成低停顿的垃圾回收是因为：它极力避免全局范围的垃圾回收，而是将java堆划分为多个大小固定的独立区域，并根据这些区域的垃圾堆积程度，在后台维护一个优先列表，每次根据停顿的时间优先回收垃圾最多的区域。
#### 垃圾收集器参数总结
